...
class TestLoader(object):
    ...
    def loadTestsFromModule(self, module, *args, pattern=None, **kws):
        ...
        tests = []
        for name in dir(module):    #EDL: Query the module attributes and try to load an object. BaseTest, TestETLLite, unittest
            obj = getattr(module, name)
            if isinstance(obj, type) and issubclass(obj, case.TestCase):    #EDL: Good stuff here.
    ...
    def discover(self, start_dir, pattern='test*.py', top_level_dir=None):
        ...
        top_level_dir = os.path.abspath(top_level_dir)  #EDL:   'd:\\DataRoot\\Project\\GitHub\\ETLite'
        ...
        if not top_level_dir in sys.path:
            ...
            sys.path.insert(0, top_level_dir)   #EDL:
        ...
        if not is_namespace:
            tests = list(self._find_tests(start_dir, pattern))  #EDL:
    ...
    def _get_name_from_path(self, path):
        if path == self._top_level_dir: #EDL: 'd:\\DataRoot\\Project\\GitHub\\ETLite\\tests'
            return '.'
        path = _jython_aware_splitext(os.path.normpath(path))   #EDL: 'd:\\DataRoot\\Project\\GitHub\\ETLite\\etlite'

        _relpath = os.path.relpath(path, self._top_level_dir)
        assert not os.path.isabs(_relpath), "Path must be within the project"   #EDL: Good check!
        assert not _relpath.startswith('..'), "Path must be within the project" #EDL: Good check!

        name = _relpath.replace(os.path.sep, '.')   #EDL: Good trick to convert name to module.
        return name

    def _get_module_from_name(self, name):
        __import__(name)    #EDL: Secret sauce.
    ...
    def _find_tests(self, start_dir, pattern, namespace=False):
        ...
        if name != '.' and name not in self._loading_packages:
            ...
            if not should_recurse:
                ...
                return
        # Handle the contents.
        paths = sorted(os.listdir(start_dir))   #EDL: test_lexer.py, test_parser.py, test_etlite.py
        for path in paths:  #EDL: ... ,tests
            full_path = os.path.join(start_dir, path)   #EDL: 'd:\\DataRoot\\Project\\GitHub\\ETLite\\tests\\test_etlite.py'
            tests, should_recurse = self._find_test_path(full_path, pattern, namespace)
            if tests is not None:
                yield tests
            if should_recurse:
                # we found a package that didn't use load_tests.
                name = self._get_name_from_path(full_path)  #EDL: 'd:\\DataRoot\\Project\\GitHub\\ETLite\\etlite'
                self._loading_packages.add(name)    #EDL: etlite, tests
                try:
                    yield from self._find_tests(full_path, pattern, namespace)  #EDL: 'd:\\DataRoot\\Project\\GitHub\\ETLite\\tests'
                finally:
    ...
    def _find_test_path(self, full_path, pattern, namespace=False):
        ...
        """
        basename = os.path.basename(full_path)  #EDL: tests, 'test_etlite.py'
        if os.path.isfile(full_path):
            ...
            name = self._get_name_from_path(full_path)  #EDL: 'tests.test_etlite'
            try:
                module = self._get_module_from_name(name)   #EDL: Jump to else: with module == BaseTest,testETLLite, unittest.
                ...
                mod_file = os.path.abspath(
                    getattr(module, '__file__', full_path)) #EDL: 'd:\\DataRoot\\Project\\GitHub\\ETLite\\tests\\test_etlite.py'
                realpath = _jython_aware_splitext(
                    os.path.realpath(mod_file))
                fullpath_noext = _jython_aware_splitext(
                    os.path.realpath(full_path))    #EDL: 'd:\\DataRoot\\Project\\GitHub\\ETLite\\tests\\test_etlite'
            ...
            try:
                ...
                load_tests = getattr(package, 'load_tests', None)   #EDL: Does it have the 'load_tests' attribute.  No it doesn't.
                # Mark this package as being in load_tests (possibly ;))
                self._loading_packages.add(name)
                try:
                    tests = self.loadTestsFromModule(package, pattern=pattern)  #EDL: package==module.  Confusing naming.
                    if load_tests is not None:
...
defaultTestLoader.discover( os.getcwd() )
